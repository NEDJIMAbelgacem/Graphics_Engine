...
private:
    ...
    std::vector<int> SubdivideIcosahedron(std::vector<int>& icosahedron) {
        std::vector<int> res;
        for (int i = 0; i < icosahedron.size(); i += 3) {
...
public:
    Icosphere(glm::vec3 _position, float _radius, glm::vec3 _color, glm::vec3 _rotation = glm::vec3(0.0f), int nb_triangles = 50) 
        : Object_3D(_position, _rotation), radius(_radius), color(_color) {
        this->vao = new VertexArray();

        faces = ConstructInitialIcosahedron();
        for (int i = 0; i < 4; ++i) faces = SubdivideIcosahedron(faces);
