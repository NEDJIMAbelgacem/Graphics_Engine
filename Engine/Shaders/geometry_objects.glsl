...
void main() {
    ...
    material_texel = u_test_param * vec4(1.0f);
    ...
    for (int i = 0; i < u_lights_count; ++i) {
        ...
        specularL += 0.05 * pow(max(dot(reflVect, toCamera), 0.0f), 1.0f) * vec4(u_lights[i].color, 1.0f) * material_texel;
    }
	// diffuse light
