...
float signed_distance(vec3 p) {
	...
	return smooth_union_operator(floor_dist, sphere_dist, 0.0f);
}
...
int closest_object(vec3 p) {
	...
	if (floor_dist < sphere_dist && floor_dist < sphere_dist2 && floor_dist < torus_dist) return 0;
...
vec3 raymarching_routine(vec3 origin, vec3 ray, out bool background_hit, out vec3 collision_point, out vec3 normal, out vec3 reflection) {
	...
	if (background_hit) return vec3(1.0f);
...
void scene1_main() {
	...
	for (i = 1; i < bouncing; ++i) {		
		vec3 object_color3 = raymarching_routine(collision_point1 + reflection1, reflection1, background_hit2, collision_point2, normal2, reflection2);
	...
	if (background_hit2) {
		...
		vec3 sky_color = texture(u_skybox, reflection1).rgb;//calculate_color(obj, collision_point2, normal2, reflection2);
		if (u_isHDR) {
